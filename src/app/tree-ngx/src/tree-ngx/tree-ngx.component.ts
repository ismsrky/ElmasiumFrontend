...
export class TreeNgxComponent implements OnInit, OnDestroy, OnChanges {
  ...
  private subscriptionEditItemObs: Subscription;
  private subscriptionNewItemObs: Subscription;
  private subscriptionDeleteItemObs: Subscription;
  ...
  @Output() newSubItem = new EventEmitter<any>();
  ...
  ngOnInit() {
    ...
    this.subscriptionEditItemObs = this.treeService.getEditItemObs().subscribe(
      x => {
        if (this.editItem) {
          this.editItem.emit(x);
        }
      }
    );
    this.subscriptionNewItemObs = this.treeService.getNewItemObs().subscribe(
      x => {
        if (this.newSubItem) {
          this.newSubItem.emit(x);
        }
      }
    );
    this.subscriptionDeleteItemObs = this.treeService.getDeleteItemObs().subscribe(
      x => {
        if (this.deleteItem) {
          this.deleteItem.emit(x);
        }
      }
    );
  }
  ...
  ngOnDestroy() {
    ...
    this.subscriptionEditItemObs.unsubscribe();
    this.subscriptionNewItemObs.unsubscribe();
    this.subscriptionDeleteItemObs.unsubscribe();
  }
